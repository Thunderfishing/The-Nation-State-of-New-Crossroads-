# Crossroads Monetary Reform & Economic Stabilization Act of 2025: Draft 8.0

**Posted to r/Bulwarkomics**
**Draft**: 8.0 | **Date**: April 10, 2026
**Author**: Thunderfish2

The Crossroads Monetary Reform & Economic Stabilization Act of 2025 (Draft 8.0) establishes New Crossroads' monetary system following the 2025 $13 trillion debt reset. It drives a $14.5 trillion GDP (65% co-ops/$9.425 trillion, 15% corporate/$2.175 trillion, 20% informal/$2.9 trillion) for 112 million citizens, targeting $38.94 trillion GDP and $1.5–2.0 trillion Sovereign Wealth Fund (SWF) by 2075, with automatic caps to prevent excessive growth.

The system supports 67 million middle-class citizens through a dual currency (physical cash + Bulwark Coin/BWC), 5,000 credit unions, the Crossroads Loan Service (CLS, 60,000 officers), and deep integration with Federated Cooperatives Limited (FCLs). The system is built on **asymmetric scrutiny**: extreme transparency for large co-ops, corporations, and elites; and for ordinary citizens, daily life that is friction-free, private, and completely their own.

The informal economy — all citizen-to-citizen commerce conducted in physical cash below the Daily Living Basket threshold — is not merely tolerated but **actively protected as a constitutional economic right**. The state has no interest in tracking it, no mechanism for doing so, and is affirmatively prohibited from building one.

---

## Overview

This act creates:
- Dual currency (physical cash + blockchain-based BWC) with asymmetric oversight and an **Informal Economy Guarantee**
- **Deflation Adjustment Index (DAI)** on all co-op, credit union, and eligible corporate loans, ensuring BWC appreciation does not penalize borrowers, with a reduced 0.5% floor for service-sector loans
- **Two-Layer Velocity Architecture**: cash handles high-frequency daily commerce; BWC serves as the store-of-value and formal economy medium
- $550 billion SWF scaling to $1.5–2.0 trillion (capped at 12% of GDP)
- $125–$175 billion reserve scaling to $407–$504 billion
- 94 million Corporate Citizens equipped with $50 BWC wallets (scaling to $326), 1,000 credit union shares (4–8% dividends), and $208 billion banking suite
- Physical cash designed as the most beautiful currency in the world, with embedded gold/silver flecks for psychological trust and counterfeiting resistance
- **Automatic SWF stabilizers** that reduce or suspend contributions if the SWF exceeds 12% of GDP

Credit unions and CLS maintain the 65/15/20 GDP split, funded by $497.5 billion annual revenue (adjusted from original $601.58 billion after eliminating co-op SWF recharge).

---

## Section 1: Monetary System

### 1.1 Dual Currency System — Two-Layer Velocity Architecture

New Crossroads operates an intentional **two-layer monetary architecture**:

- **Layer 1 — Cash (Velocity Layer)**: High-frequency, private, friction-free. Handles all daily citizen commerce. Designed for maximum turnover. No surveillance, no fees, no records.
- **Layer 2 — BWC (Store-of-Value / Formal Layer)**: Lower-frequency, transparent above $10,000, deflationary by design. Handles institutional transactions, large purchases, savings, and international trade.

These layers are complementary, not competing. Together they capture the full range of economic activity without forcing any citizen into the surveillance architecture to buy groceries.

---

#### Physical Cash — Layer 1

**Rationale**: Cash is the velocity engine of the New Crossroads economy. It exists to be spent, passed hand to hand, and spent again. The state's goal is not to track it but to ensure there is always enough of it, that it is trusted and beautiful, and that no citizen is ever penalized for using it. The informal sector — all commerce conducted in cash below the Daily Living Basket threshold — is a protected constitutional economic zone.

- **Volume (2025)**: $1.45 trillion (10% of $14.5T GDP), scaling to $4.67 trillion (12% of $38.94T GDP) by 2075
- **Composition**: Polymer substrate with embedded gold and silver flecks (approx. 5–10% of face value by metal content, varying by denomination)
- **Design**: Holographic security features, regional artistic motifs (e.g., Corridon barges, Frostpeak mountains, Ember Range geothermal fields), ranked among the world's most beautiful currencies
- **Backing**: Embedded metal flecks provide psychological trust and intrinsic value floor. Full backing via convertibility to BWC and state acceptance for taxes and fees.
- **Denominations**: $1, $5, $10, $20, $50, $100 BWC — adjusted biennially to the **Daily Living Basket (DLB)** index (see Section 1.5) by Central Council (6/11 vote), ensuring the cash layer always covers genuine daily commerce
- **Swap Fee (Cash → BWC)**: **Free**. No fee to enter the formal system from cash.
- **Swap Fee (BWC → Cash)**: 1% ($14.5 billion/year in 2025, scaling to $46.7 billion/year by 2075). Encourages daily spending to remain in cash; the friction is on exit from formal, not entry.
- **Legal Export**: **Strictly prohibited**. Physical cash may not be taken across national borders. Customs enforcement at all ports (97 million tons capacity, Regions 12, 18) and land crossings. Violations subject to seizure and fines up to 10x face value.
- **Counterfeiting Resistance**: Embedded metal flecks require actual precious metal; polymer substrate requires industrial-scale equipment; holographic features require optical expertise. The barrier to entry is so high that only state actors could counterfeit, and the state has no incentive.
- **Cost**: $500 million/year minting and distribution (0.034% of cash volume), offset by BWC-to-cash swap fees

---

#### Bulwark Coin (BWC) — Layer 2

**Rationale**: BWC is the store of value and formal economy medium. It is fully backed by the reserve, deflationary by design (appreciating ~2.6%/year from $50/BWC to ~$300/BWC by 2075), and transparent for large transactions. Because BWC appreciates in real terms, borrowing in BWC carries an implicit real cost above the nominal interest rate. The **Deflation Adjustment Index (DAI)** (see Section 1.6) corrects for this on all qualifying loans, ensuring the deflationary design does not choke investment or punish co-ops.

- **Type**: Blockchain-based digital currency
- **Reserve**: $125–$175 billion (30% gold, 30% palladium, 15% platinum, 10% iridium, 10% rhodium, 5% ruthenium), scaling to $407–$504 billion by 2075
- **Backing Ratio**: 100%+ (over-backed, deflationary by design)
- **Appreciation Rate**: ~2.6%/year average (0.025 oz → ~0.0875 oz backing per BWC by 2075); published annually as the **BWC Appreciation Rate (BAR)** by Treasury
- **Transaction Fees**:
  - 0% for transactions under $100 BWC
  - 1% for transactions $100–$10,000 BWC
  - 1.5% for transactions $10,001–$100,000 BWC *(smoothed from prior 2% cliff to reduce bunching behavior)*
  - 2% for transactions above $100,000 BWC
- **Transparency Threshold**: All BWC transactions above $10,000 are logged in the CWD and subject to audit. Transactions below $10,000 are private and unlogged.
- **Features**: Real-time reserve ratio visible on wallets, no burns, CME-proof via 400 TWh nuclear grid and 10 vaults ($100 million)
- **Purpose**: Facilitates FCL payments (~$112,000/year per participant), large co-op and corporate transactions, and international trade via CGCI

---

### 1.2 Co-op Credit Union Network

**Rationale**: Credit unions are the backbone of the system — member-owned, locally governed, and subject to Regional Board audit. Large loans face extreme scrutiny; micro-loans are issued with minimal friction. The credit union network is the **formal handshake with the informal sector**: it offers credit to citizens whose income is real but conducted in cash, without requiring them to prove or disclose that income beyond self-attestation for micro-loans.

- **Structure**: 5,000 credit unions (250 per region) issue BWC (0.025 oz reserve backing).
- **Assets Managed** (2025 → 2075):
  - Loans: $452.5 billion → $1.2084 trillion
  - Banking suite: $208 billion
  - Wallets: $15.04 billion
  - Shares: $94 billion

- **Loan Allocation** ($452.5 billion total, $22.625 billion/region):
  - 65% co-op: $294.125 billion
  - 15% corporate: $67.875 billion
  - 20% informal: $90.5 billion
  - Non-SWF loans: $50 billion ($10 million/region, 50% to co-ops <$5 million, 5–6% interest, DAI-adjusted)
  - Micro-loans: $10 billion ($500 each, 4% interest, self-attested income, no verification required)

**Asymmetric Scrutiny**:
- Loans below $500,000: streamlined approval, minimal documentation, issued by CLS agents within 48 hours
- Loans $500,000–$1 million: standard review, Regional Board notification
- Loans above $1 million: full audit, CWD tracking, Regional Board approval (6/11 vote), public disclosure
- Loans above $10 million: Central Council review (6/11 vote), SAP arbitration if disputed, published in annual transparency report

- **Banking Suite** ($208 billion):
  - Checking: $94 billion (1% fee, waived for $10,000 earners)
  - Savings: $94 billion (5–6%)
  - Mutual funds: $10 billion (4.6%)
  - GICs: $5 billion (3–5%)
  - Bonds/FCL equity: $5 billion (3–4%)
  - Savings bonds: $47 billion (3–4%)
  - Crypto: $23.5 billion (5–10%)
  - Retirement: $94 billion (6–8%)

- **Wallets**: $15.04 billion ($50 per citizen, scaling to $326 by 2075; ETF limits $10,000–$20,000/BWC). Contributions up to $5,000; withdrawals $500/year ($250 low-income, $750 high), $1,500 post-65.
  - **Asymmetric Scrutiny**: Wallet balances below $10,000 are private. Balances $10,000–$100,000 visible to CWD for anti-fraud aggregate tracking only. Balances above $100,000 trigger automatic audit.

- **Shares**:
  - Base: $94 billion ($1/BWC, max 1,000 per citizen, 4–8% dividends)
  - Special: $5,000–$25,000 (5–6% dividends, Gold/Silver status with 0.5 advisory vote on non-SWF loans)

- **Revenue Breakdown** ($96.6185 billion/year total):

| Source | Amount ($ billion/year) | Notes |
|--------|--------------------------|-------|
| BWC Transaction Fee | 12.0 | 0% under $100, 1% $100–$10,000, 1.5% $10k–$100k, 2% above $100k |
| Cash Swap Fee (BWC→Cash only) | 14.5 | 1% on BWC-to-cash conversions; Cash→BWC is free (2025; scales to $46.7B by 2075) |
| Checking Account Fee | 0.94 | 1% on balances, waived for $10,000 earners |
| ETF Transaction Fee | 0.705 | 0.5% on ETF investments |
| Liquidity Fee | 2.2625 | 0.5% on loan disbursements, 1% if low liquidity |
| Micro-Loan Fee | 0.2 | $10/BWC per micro-loan |
| Micro-Loan Interest | 0.4 | 4% on $10 billion micro-loans |
| Non-SWF Loan Interest | 2.5 | 5–6% on $50 billion non-SWF loans (DAI-adjusted nominal rate) |
| Insurance Commissions | 0.5 | 5% on partnerships |
| SMSWF Profits | 2.48 | From mining FCLs |
| Reserve Interest / Other SWF | 71.2685 | ~2.4% on $125–$175 billion reserve + other investments |
| **Total** | **107.6185** | |

- **Crossroads Loan Service (CLS)**
  - Officers: 60,000 (3,000/region)
  - Structure: 48,000 agents ($75,000/year + $12,000 stipend + 3% shares), 11,800 seniors ($120,000/year), 200 regional ($200,000/year)
  - Academy: $1.5375 billion/year (Crossroads City + New Tech City campus); trains in loan management, student ventures, bankruptcy recovery, **and informal sector engagement**
  - Functions: Manage $452.5 billion loans, issue $500 student venture loans (ages 12–15, lenient criteria, mentorship for successful ventures), support bankruptcy recovery, ensure 65/15/20 GDP split
  - **Informal Sector Mandate**: CLS agents are trained and incentivized to serve informal sector participants. Micro-loans ($500) are issued on self-attested income with no cash income verification. CLS agents may not require disclosure of cash transaction history as a condition of any loan under $500,000.
  - **Asymmetric Scrutiny**: CLS agents have discretion for loans under $500,000. Loans above $1 million require senior CLS approval and CWD logging. Loans above $10 million require regional CLS director approval and public disclosure.
  - Cost: $1.875 billion/year ($93.75 million/region)
  - Funding: $1.5375 billion credit union revenue + $337.5 million SWF
  - Oversight: CWD tracks performance, citations (5–10% salary bumps), disciplinary actions (5–15% fines/suspensions/expulsion)

---

### 1.3 Transaction Fees & Liquidity Pool

**Rationale**: Fee structure encourages cash for daily use, BWC for medium transactions, and progressive fees for large transfers to prevent elite avoidance. The smoothed BWC fee tiers (0% / 1% / 1.5% / 2%) eliminate the sharp cliff at $10,000 that previously encouraged transaction-splitting behavior. Wartime fees provide emergency funding without crushing ordinary citizens (cash exempt entirely).

- **BWC Fees**: 0% under $100; 1% $100–$10,000; 1.5% $10,001–$100,000; 2% above $100,000 (Central Council may adjust bands biennially, 6/11 vote)
- **Cash Swap**: Cash→BWC: **free**. BWC→Cash: 1% ($14.5 billion/year in 2025, scales to $46.7B by 2075)
- **Wartime Fee**: 10% on BWC transactions above $10,000 only ($500–725 billion/year, 5-year cap, 7/11 renewal); cash and cash-to-BWC swaps exempt entirely
- **Liquidity Pool**: 0.5% fee ($500 million/year), 1% if cash tight; supports rural families/projects
- **Vaults**: 10 vaults ($100 million) store BWC snapshots for CME reboot

---

### 1.4 Personal Market Index (PMI)

**Rationale**: The PMI provides real-time tracking of the 65/15/20 GDP balance, allowing early intervention before co-op GDP dips below 60%. Critically, the informal sector component of the PMI is estimated via statistical sampling — **not** transaction surveillance. Adjustments are automatic and transparent, visible on every citizen's wallet.

Tracks 65/15/20 GDP balance, informal sector growth (estimated via quarterly household sampling surveys, not CWD tracking), rent/gas prices, jobs, and digital shifts via real-time BWC ratio tied to $125–$175 billion reserve.

If co-op GDP dips below 60%, Treasury triggers fee hikes (1–2% on BWC transactions above $1,000 only) or cash shifts to pensions/emergencies. Adjustments visible on wallets; logged on CWD. **The informal sector's 20% GDP share is a floor, not a cap** — growth beyond 20% is welcome and requires no policy response.

---

### 1.5 Daily Living Basket (DLB) — Informal Threshold Index

**Rationale**: The threshold below which cash transactions constitute protected informal commerce must track real purchasing power, not nominal values. A $100 threshold that covers a restaurant meal in 2025 should still cover a restaurant meal in 2050. Indexing to a defined basket of common goods ensures the informal sector always covers genuine daily life.

- **Definition**: The DLB is a defined basket of common goods and services representing one week of ordinary household commerce: groceries (for 2 persons, 7 days), one restaurant meal, one tank of fuel, one week of transit fares, and one standard household utility payment.
- **Base Value (2025)**: $100 BWC
- **Adjustment**: Recalculated annually by Treasury statisticians. Published in the first week of January. Automatically updates the cash transaction privacy threshold, BWC 0% fee band, and DLB-referenced loan terms.
- **Governance**: DLB methodology reviewed by Central Council every 5 years (6/11 vote). Methodology is public. Basket composition changes require Regional Board majority (11/20).
- **Effect**: As BWC appreciates, the DLB threshold in BWC terms will generally decline (i.e., $100 in 2025 BWC buys more in 2050 BWC terms). The DLB recalculation counteracts this, keeping the informal layer genuinely covering daily life.

---

### 1.6 Deflation Adjustment Index (DAI) — Loan Rate Correction

**Rationale**: BWC is designed to appreciate approximately 2.6%/year in real terms over the 50-year horizon ($50/BWC → ~$300/BWC by 2075). This is a feature for savers and a potential problem for borrowers: a co-op borrowing 1,000 BWC at 5% nominal faces an effective real borrowing cost of ~7.6% once BWC appreciation is accounted for. Without correction, deflation would systematically discourage investment — exactly the wrong outcome for a growing co-op economy. The DAI corrects for this by adjusting the **nominal interest rate** on qualifying loans downward by the published BWC Appreciation Rate (BAR), so that real borrowing costs remain stable and predictable.

#### How the DAI Works

- **BWC Appreciation Rate (BAR)**: Published annually by Treasury in January. Calculated as the trailing 3-year average annual appreciation of BWC's reserve backing (oz per BWC). Smoothed to prevent single-year volatility from distorting loan rates.
- **DAI-Adjusted Rate**: `Nominal Rate − BAR = DAI-Adjusted Effective Rate`
  - Example (2025): Nominal rate 5%, BAR 2.6% → DAI-adjusted rate 2.4%
  - The borrower repays in BWC at the nominal rate, but the real burden is calibrated to 2.4%
- **Floor**: DAI-adjusted rates may not fall below 1% for standard loans, **or 0.5% for service-sector loans** (healthcare, education, infrastructure), regardless of BAR. Prevents zero or negative real rates while acknowledging the higher labor subsidy in service sectors.
- **Publication**: DAI rate table published annually alongside BAR. All qualifying loan contracts reference the BAR year of origination, locked for the loan term.

#### Qualifying Loans

| Borrower Type | DAI Application | Floor |
|---|---|---|
| **Co-ops (all FCLs)** | Mandatory | 1.0% |
| **Credit Union non-SWF loans** | Mandatory | 1.0% |
| **Micro-loans** | Mandatory | 0.5% |
| **Informal sector loans** | Mandatory | 0.5% |
| **Service-sector loans (healthcare, education, infrastructure)** | Mandatory | 0.5% |
| **Corporate loans** | Optional | 1.0% |
| **Special shares** | Not applicable | — |

#### Secondary Market for Special Shares

To improve liquidity without unlocking the formal capital base, citizen-members may trade special shares on an internal **Credit Union Secondary Market (CUSM)**:
- Trades permitted between citizen-members only. Foreign nationals and non-members excluded.
- No cash-out: shares transfer between member accounts; no BWC is extracted from the credit union system.
- Price discovery: market-determined within a band of 90%–110% of par value. Trades above par require CLS notification (no approval needed, notification only).
- Governance: CWD logs secondary market aggregate volume only. Individual trades are private.
- Effect: Citizens who need liquidity can sell shares to willing buyers rather than forcing early withdrawal ($500/BWC/year cap), keeping capital deployed in the loan system while giving individuals flexibility.

#### Circuit Breaker for Rapid BWC Appreciation

If BAR exceeds 3% for three consecutive years, the Treasury shall automatically:
- Reduce all DAI floors to 0.25% for the following year
- Publish a public notice of "Deflation Caution" with recommended borrowing incentives
- Suspend the 1% BWC→Cash swap fee for 12 months to encourage liquidity

This circuit breaker prevents deflation from unexpectedly choking investment during periods of rapid currency appreciation.

---

## Section 1A: Informal Economy Guarantee

**This section establishes the Informal Economy Guarantee as an affirmative constitutional and legislative protection for cash-based commerce. It is not merely an administrative policy — it is a binding prohibition on state surveillance of ordinary economic life.**

### 1A.1 Declaration

The state of New Crossroads declares that:

1. All citizen-to-citizen commerce conducted in physical cash at or below the current Daily Living Basket threshold constitutes **protected informal economic activity**.
2. Citizens engaged in such commerce have an **absolute right to privacy** that no government agency, credit union, or FCL may abridge, compel disclosure of, or use as a condition of access to any service.
3. The state has **no legitimate interest** in tracking, aggregating, or reporting informal sector transactions below the DLB threshold, and affirmatively **prohibits** the construction of any system designed to do so.

### 1A.2 Specific Prohibitions

The following are unlawful for any government body, credit union, CLS officer, FCL, or corporate entity:

- Requiring disclosure of cash transaction history as a condition of any loan, membership, banking service, or government benefit
- Conditioning micro-loan approval on proof of formal income where cash income is self-attested
- Using cash holdings or cash transaction patterns as evidence of tax evasion absent independent corroborating evidence
- Constructing databases, AI systems, or surveillance infrastructure designed to track cash transactions below the DLB threshold
- Refusing service to citizens who decline to disclose cash income

### 1A.3 Credit Union Non-Discrimination Requirement

Credit unions may not:
- Deny membership, downgrade tier status, or withhold dividends based on a member's ratio of cash to BWC activity
- Require BWC transaction history to qualify for micro-loans or informal sector loans
- Report member cash withdrawal patterns to any government body absent a court order

### 1A.4 Informal Sector Micro-Loan Access

Citizens whose primary income is cash-based (informal sector participants) receive automatic access to:
- **Micro-loans** ($500, 4% nominal / ~1.4% DAI-adjusted): Self-attested income, no documentation required. CLS agents process within 24 hours.
- **Base shares** (1 free share on first micro-loan repayment, as a credit union welcome incentive)
- **Basic Wallet** (BWC Layer 2 access without requiring formal income history)

The purpose is to ensure the formal financial system is always accessible to informal sector participants on their terms — not to pull them into surveillance.

### 1A.5 Sampling-Only Measurement

The state shall measure the informal sector's contribution to GDP exclusively through:
- Voluntary quarterly household expenditure surveys (anonymized, opt-in)
- Statistical extrapolation from DLB price tracking
- Aggregate cash minting and circulation data (no individual tracking)

No surveillance-based method may be substituted for statistical sampling. The informal sector GDP figure (20% floor) is an estimate, and the legislation accepts this uncertainty as the appropriate price of privacy.

---

## Section 2: Sovereign Wealth Fund (SWF)

### 2.1 Funding Mechanisms

**Rationale**: The SWF is the collective wealth of 112 million citizens. Every transaction, tax, and resource flow is transparently tracked at the institutional level. Routine operations are automated; large withdrawals and investments face multiple layers of approval. The SWF is designed to be **self-limiting** — it will never exceed 12% of GDP, and automatic stabilizers reduce contributions when it approaches this cap.

- **Initial**: $550 billion (2025)
- **Target (2075)**: $1.5–2.0 trillion (reduced from original $3.08T to prevent ballooning)
- **Cap**: 12% of GDP (automatic citizen dividend above this level)
- **Recharge**: **Eliminated** — co-ops no longer pay 2% of GDP into SWF
- **Annual Contributions** (revised):
  - Financial transactions tax (0.3% on BWC >$10k): $150 billion/year
  - Corporate SWF contribution (5% of profit over $1M): $60 billion/year
  - Resource surcharge (3% on minerals/energy): $50 billion/year
  - Other (fees, tariffs, etc.): $102.5 billion/year
  - **Total annual SWF contribution**: ~$362.5 billion/year

- **Sub-funds**:

| Sub-fund | Initial Amount ($ billion) | Purpose / Key Allocations | Scrutiny Level |
|----------|----------------------------|---------------------------|----------------|
| Charity | 10.1 | $2B tax credits, $2B injury payouts, $300M rural bonuses | Public disclosure below $1M |
| Rainy-Day | — | Buffer for economic shocks (fuel spikes, etc.) | Central Council oversight |
| Emergency | 50 | Crises (flooding, etc.), wartime fee boosts | 3/20 EGA trigger for release |
| Defense | 4.14/year | Military assets, boot camp bonuses | Classified above certain levels |
| Healthcare | — | Co-op clinics for 70 million citizens | Regional Board audit |
| Mental Health | — | Therapy programs | Regional Board audit |
| Education | — | 18 million students via co-op schools | NLC oversight |
| Workforce | — | Boot camp pipelines for 13 million journeymen | CWD tracking |
| Military-Industrial | — | Military assets | Classified |
| Pension | — | Elder care | Automatic distribution |
| Research/Tech | — | Fusion, AI innovation | $50M+ requires Central Council |
| Communications | 15 | Broadband initiatives | Regional Board approval |
| Forced Savings | 94 | CCIF savings (5–6%, $25,000 cap), peaks SWF at $1.25T | Individual accounts private |
| Housing | 150/year | Co-op leases ($75,000 withdrawals), private housing grants | $75k automatic; $150k+ review |
| Special Mining (SMSWF) | 25 by 2045 | Funds 25 mining FCLs → 250 by 2125 ($5B profit/year) | Full CWD tracking |

### 2.2 Debt Jubilee & Additional Funding

**Rationale**: The 2025 debt reset and 25-year co-op debt jubilees prevent the debt accumulation that destroyed the previous system. Freedom shares issued by Regional Board vote ensure democratic control.

- **Jubilee**: 2025 $13 trillion debt wipe; 5-year bankruptcy reset. Every 25 years, 50% co-op debt ($50 billion) forgiven; freedom shares issued (11/20 Regional Boards vote).
- **Seeding** (revised): $362.5 billion/year from new tax structure (no co-op recharge)
- **Growth Drivers**: Reserve ETF ($1.25 billion/year), CCIF ($2.395 trillion), housing ($150 billion/year), Crossroads Global Co-op Index ($329 billion/year)

### 2.3 Automatic SWF Stabilizers (New)

To prevent excessive SWF growth, the following automatic mechanisms apply without requiring legislative action:

| SWF/GDP ratio | Automatic action |
|---|---|
| < 8% | Normal operations; full contributions |
| 8–10% | Normal operations; full contributions |
| 10–12% | Warning zone; no action but public notification |
| **12% (Cap reached)** | **Automatic citizen dividend** — 50% of annual excess above 12% distributed equally to all citizens (cash and BWC, 50/50 split) |
| 12–14% | Reduce all SWF contribution rates by 25% |
| 14–16% | Reduce all SWF contribution rates by 50% |
| >16% | Suspend all SWF contributions entirely until ratio falls below 12% |

**Citizen Dividend Mechanics**:
- Calculated annually by Treasury on January 15
- Paid in April (50% physical cash, 50% BWC)
- Cash portion is untracked and constitutionally protected
- BWC portion is deposited directly into citizen wallets
- No tax liability on dividends

**Contribution Rate Cuts** apply proportionally to:
- Financial transactions tax (rate reduced from 0.3% to 0.225%, 0.15%, or 0%)
- Corporate SWF contribution (rate reduced from 5% to 3.75%, 2.5%, or 0%)
- Resource surcharge (rate reduced from 3% to 2.25%, 1.5%, or 0%)

### 2.4 Contribution Phase-Out

By 2075–2100, the SWF is designed to **wean itself off contributions**:

| Phase | Years | Annual contribution | SWF growth driver |
|-------|-------|---------------------|-------------------|
| Build | 2025–2050 | $362.5B | Contributions + returns |
| Stabilize | 2050–2075 | $150B | Mostly returns |
| Mature | 2075–2100 | $0 | Returns only |
| Dividend | 2100+ | Negative (distributions exceed returns during booms) | Smoothing only |

At maturity, the SWF acts solely as a **buffer** — growing in bad years (when returns are negative and no dividends paid), shrinking in good years (when dividends distribute excess). It never balloons.

### 2.5 Specific SWFs — Detailed Examples

- **Special Mining SWF (SMSWF)**:
  - Target: $25 billion by 2045 ($1.25 billion/year ETF)
  - Funds: National Crossroads Silver Corporation (NCSC, $500 million/year)
  - Operations: 25 mining FCLs (250,000 oz gold at $400/oz cost → $2,000/oz sale; 2.5 million oz silver at $4/oz cost → $32/oz sale; $470 million profit/year)
  - Profit Split: 70% to credit unions ($330.4 million), 30% to NCSC ($141.6 million: $50M reserve, $91.6M SMSWF)
  - Scale: 250 FCLs by 2125 ($5 billion profit/year)
  - Pilot: 5 FCLs ($100 million)
  - Streams: 60% gold, 20% silver, 10% palladium, 10% platinum
  - **Metal Allocation for Cash**: 40% of streamed gold/silver allocated to physical cash minting (gold/silver flecks for polymer notes)

- **Forced Savings & Housing**:
  - Savings: 94 million citizens save 10% ($705 billion/year, 4% return). Informal earners under $100,000 opt out tax-free; $1,000 share bonus for opt-in. Yields $117.5 billion by 2030, $47 billion reserves for loans.
  - Housing: $150 billion/year supports $75,000 co-op lease withdrawals ($82,000 cost, 1.8 million units/year, $7,000 subsidy), private housing ($90,000, 0.2 million units/year, $10,000 grants). Targets 86% homeownership (96 million) by 2085. $5,000/child incentive ($15 billion/year) boosts fertility to 1.8.

---

## Section 3: Co-op Ecosystem Integration

**Rationale**: FCLs are the engine of the 65% co-op economy. Their profits are transparently split. Large FCLs face full CWD tracking; small FCLs operate with minimal overhead. All FCL loans are DAI-adjusted, ensuring the deflationary BWC does not systematically advantage lenders over co-op borrowers.

- **FCL Role**: **195,100 FCLs** (9,755/region) drive $9.425 trillion (65% GDP), scaling to $25.311 trillion by 2075. *(Correction: Original 27,100 was insufficient for 112–130M population; 195,100 FCLs represents approximately 575–670 citizens per FCL, consistent with cooperative density benchmarks.)*
- **Contribute**: $2.036 trillion exports, $274 billion resources.
- **Profit Split**: 10% healthcare, 10% education, 10% charity, 30–40% member/employees, 30–40% investors/founders; adjustable via 75% vote.
- **Capital Investment Fund (CCIF)**: $2.395 trillion ($1.96 trillion FCLs, $435 billion corporates), 7.5% returns, 0.5% fee. Funds co-op startups, SWF projects.
- **Alliance Network**: 1,832 members (1,755 FCL-elected, 77 Regional Board appointees) allocate CCIF funds, approve investments (65% FCL vote + 11/20 Board confirmation). Supports fusion, telehealth, media, parkweb, energy hubs. Audited via CWD.
- **Asymmetric Scrutiny**: FCLs with revenue below $1 million file simplified annual reports. FCLs above $1 million file full CWD disclosures. FCLs above $100 million face quarterly audits and public transparency hearings.
- **Oversight**: 50–75 auditors + $1 billion blockchain/AI cap $2.5 billion fraud.

---

## Section 4: Tax and Economic Reforms (Revised)

**Rationale**: The tax system is progressive but simple. Informal earners under $100,000 pay no tax and have no filing obligation. Large corporations face higher rates. Co-ops pay a lower profit tax (5%) and no SWF contribution. All tax data is transparent to the individual but not publicly disclosed.

- **Total Revenue** (2025): $497.5 billion (revised from $601.58B after eliminating co-op SWF recharge)
- **Tax Breakdown** (revised):

| Tax Type | Rate / Threshold | Yield ($ billion/year) | Scrutiny Level |
|----------|------------------|------------------------|----------------|
| Personal Income Tax (formal wages only) | 5% flat on FCL/corporate wages/shares; 0% under $20,000; **informal wages 0% (exempt)** | 115 | Automatic withholding; no filing for most |
| Co-op Profit Tax | **5%** on profit over $100,000; 0% under $100,000; **no SWF contribution** | 45 | Full CWD disclosure for co-ops >$1M |
| Corporate Profit Tax | 0% under $100,000; 15% $100k–$1M; 20% over $1M | 25 | Full CWD disclosure |
| Informal Business Tax | 0% under $100,000 profit; 15% $100k–$1M; 20% over $1M (self-attested, no surveillance) | 10 | Self-attested annual filing; no audits below $100k |
| Excise Tax | 3% on $500 billion fuel/goods | 15 | Collected at point of sale; no individual tracking |
| Property Tax | 0.75% on $2.5 trillion commercial/industrial | 18.75 | Assessed annually; public record |
| Tariffs | 9.8% avg. on $500 billion imports | 49 | Collected at port; transparent |
| Fees/Patronage | Various (credit unions + other) | 15.65 | Minimal scrutiny below $10k |
| Resources | Minerals, agriculture, fuels, etc. | 274 | Full CWD tracking for extractive industries |
| **Subtotal** | — | **567.4** | |
| *Less: Eliminated co-op SWF recharge* | *Previously $290B* | *(−290)* | *—* |
| **Total** | — | **277.4** | *(Taxes/fees only)* |
| **Plus SWF-dedicated revenue** (see 2.1) | FTT + Corp SWF + Resource surcharge | **+220** | |
| **Grand Total** | — | **497.4** | |

- **Allocations**: $50 billion infrastructure, $30 billion R&D, $20 billion agro camps, $20 billion housing, $64.25 billion reserve, $50 million/year parkweb, $500 million/year greenhouses.
- **Asymmetric Scrutiny**: All government allocations above $1 million are publicly disclosed in the CWD and subject to Regional Board audit. Allocations below $1 million are administered automatically with quarterly sampling audits.
- **Oversight**: Treasury + 50 agents ($500 million/year) + 50–75 auditors + $1 billion blockchain/AI via CWD cap $2.5 billion fraud; recover $20 billion/year evasion.

---

## Section 5: Governance & Oversight

**Rationale**: Oversight is layered and redundant. Large transactions and elite actors face full transparency; ordinary citizens face next to none. The CWD tracks only what is necessary to prevent fraud and capital flight. The informal economy is measured by statistical inference, never by surveillance.

- **Bodies**: 11-member Central Council, 20 Regional Boards (220 members: 200 masters, 20 wildcards), 10 National Assembly grandmasters, judiciary.
- **Asymmetric Oversight Architecture**:
  - **Informal Level (Daily Living Basket and below)**: No CWD tracking. No transaction fees for cash. No reporting requirements. Constitutionally protected. Measured by sampling only.
  - **Citizen Level (Under $10,000/year BWC)**: No CWD tracking. No transaction fees for cash. No reporting requirements. Complete privacy.
  - **Middle Level ($10,000–$100,000/year BWC)**: CWD tracks aggregate totals only. No transaction-level detail. Annual summary reporting.
  - **Elite Level (Above $100,000/year or $1M assets)**: Full CWD transaction tracking. Quarterly audits. Public disclosure of holdings above $10M. Subject to SAP review if irregularities found.
  - **Institutional Level (FCLs, corporations, credit unions above $10M revenue)**: Full transparency. Public annual reports. Quarterly audits. Regional Board oversight. Central Council review for major decisions.
- **Treasury**: Oversees BWC, $550 billion SWF, $125–$175 billion reserve ETF ($1.25 billion/year). Publishes BAR and DAI rate table annually each January.
- **CWD**: $150 million/year tracks CLS performance, student venture loans, FCL metrics, citizen investments ($2,600 wallet, $500,000 loan). **Citizen-level data is anonymized and aggregated; individual transaction data for amounts under $10,000 is permanently deleted after 30 days. Cash transaction data is never collected.** Audited by 50–75 auditors + $1 billion blockchain/AI, capping $2.5 billion fraud; appeals to judiciary.

---

## Section 6: Foreign Investment — Crossroads Global Co-op Index (CGCI)

**Rationale**: Foreign capital is welcome but strictly non-voting. All foreign investment is tracked at the highest scrutiny level. Foreign nationals must use BWC or CGCI units — they may not hold physical cash, which cannot legally leave the country in any case.

- **Purpose**: Channels foreign capital into 65% co-op economy ($9.425 trillion 2025 → $25.311 trillion 2075).
- **Structure**: Treasury-managed, $50 billion SWF seed. **195,100 FCLs** pool 10% profits → $329 billion/year (2025) → $879 billion/year (2075).
- **Units**: Non-voting, 5% return ($16.45–$43.95 billion/year). 90% profits: 70% CCIF, 20% Rainy-Day Fund, 10% SWF.
- **Timeshares**: 10,000 resort condos/cottages ($725 million/year), foreign ownership cap, supports tourism (Corridon, Ember Range).
- **Scrutiny**: All foreign investment above $100,000 is logged in CWD and subject to Regional Board review (11/20 vote). Foreign nationals may not hold physical cash and must use BWC or CGCI units.
- **Governance**: 20 Regional Boards approve FCL opt-ins (11/20 vote, wildcard input). 50 auditors cap $2.5 billion fraud via CWD.

---

## Key Stats (2025–2075)

| Metric | 2025 Value | 2075 Projection |
|--------|------------|-----------------|
| Population | 112 million | 130 million |
| GDP | $14.5 trillion | $38.94 trillion |
| Informal GDP (floor) | $2.9 trillion (20%) | $7.8 trillion (20% floor) |
| SWF | $550 billion | **$1.5–2.0 trillion** (capped at 12% of GDP) |
| Reserve | $125–$175 billion | $407–$504 billion |
| Loans | $452.5 billion | $1.2084 trillion |
| DAI-Adjusted Loan Rate (standard) | Nominal −2.6% (min 1.0%) | Nominal −BAR (min 1.0%) |
| DAI-Adjusted Rate (service-sector) | Nominal −2.6% (min 0.5%) | Nominal −BAR (min 0.5%) |
| Credit Unions | 5,000 | — |
| CLS Officers | 60,000 | — |
| FCLs | **195,100** (9,755/region) | — |
| CGCI | $1 trillion cap, $329B/year | $2.5 trillion cap, $879B/year |
| Revenue (total government) | $497.5 billion | $1.35 trillion (scaled) |
| Physical Cash in Circulation | $1.45 trillion (10% of GDP) | $4.67 trillion (12% of GDP) |
| Cash Metal Content (approx.) | $72.5–$145 billion (5–10%) | $233–$467 billion (5–10%) |
| DLB Threshold (base) | $100 BWC (2025) | Adjusted annually |
| BWC Appreciation Rate (BAR) | ~2.6%/year (target) | Published annually January |
| SWF/GDP Cap | — | **12%** (automatic dividends above) |

---

## Summary of Changes (Draft 7.0 → 8.0)

| Section | Change | Rationale |
|---------|--------|-----------|
| **Overview** | SWF target reduced from $3.08T to $1.5–2.0T; added automatic stabilizers | Prevents SWF ballooning beyond 12% of GDP; addresses long-term projection showing problematic growth |
| **Section 1.6 DAI** | Added service-sector floor of 0.5% (healthcare, education, infrastructure); added circuit breaker for rapid BWC appreciation (>3% for 3 years) | Recognizes labor subsidy from Education Act; prevents deflation from choking investment during rapid appreciation |
| **Section 1.6 DAI** | New subsection: Circuit Breaker | Automatic relief when BWC appreciates too quickly |
| **Section 2.1** | Eliminated co-op SWF recharge ($290B/year); replaced with FTT + corporate SWF contribution + resource surcharge | Co-ops no longer pay into SWF; corporations share the burden |
| **Section 2.1** | Added SWF cap at 12% of GDP with automatic citizen dividends | Prevents excessive concentration of wealth |
| **Section 2.3** | New section: Automatic SWF Stabilizers (tiered contribution cuts) | Self-correcting mechanism; no legislative action needed |
| **Section 2.4** | New section: Contribution Phase-Out (2025–2100) | SWF weans itself off contributions over time |
| **Section 3** | FCL count corrected: **195,100** (not 27,100) | 27,100 was far too low for 112–130M population; corrected to ~575–670 citizens per FCL |
| **Section 4** | Co-op profit tax reduced from 12.5% to 5%; added $100k exemption | Ensures co-ops pay lower effective rate than corporations |
| **Section 4** | Informal wages explicitly 0% tax (strengthened from previous exemption) | Preserves Informal Economy Guarantee |
| **Section 4** | Corporate tax simplified to 15%/20% tiers | Simpler, more progressive |
| **Section 4** | Total revenue adjusted to $497.5B (from $601.58B) | Reflects eliminated co-op SWF recharge |
| **Section 5** | No changes | Asymmetric oversight architecture remains intact |
| **Throughout** | Removed all "a€" symbol artifacts | Clean text |
| **Key Stats** | Updated SWF target, DAI floors, FCL count, revenue | Reflects all changes |

---

## Design Philosophy (Informative)

The Crossroads monetary system is built on a single principle: **trust through asymmetry**.

The informal economy is not a gap in the system. It is not a problem to be solved, a tax base to be captured, or a population to be graduated into formal finance. It is the **living core of daily economic life** — the neighbor who fixes your fence, the market stall, the babysitter, the handshake deal. It is the part of economic life that has always worked best when the state left it alone.

New Crossroads leaves it alone. Not passively — actively and legislatively. The prohibition on cash surveillance is not a privacy policy. It is a declaration of what the state is for.

- **Ordinary citizens** face next to no scrutiny. Their cash is private, their small transactions are unlogged, their wallets are their own. The beautiful metal-flecked notes they carry are a daily reminder that the state trusts them — and they trust the state in return.

- **The formal system exists to serve the informal**, not replace it. Credit unions offer micro-loans without income verification. CLS agents process applications in 24 hours. The swap into BWC is free. The door is always open — and citizens choose when to walk through it.

- **Elites, large co-ops, and corporations** face extreme transparency. Every large transaction is logged. Every major loan is audited. Every FCL profit is disclosed. This prevents the concentration of wealth and power that destroyed the previous system.

- **The DAI ensures deflation is a feature, not a trap.** BWC appreciates because it is backed by real metals. That appreciation benefits savers. The DAI ensures it does not punish co-op borrowers by making the real cost of productive investment invisible and punishing. The new 0.5% floor for service sectors recognizes that healthcare, education, and infrastructure workers receive substantial non-monetary compensation through the Education Act's labor subsidy.

- **The SWF is designed to be self-limiting.** It will never grow beyond 12% of GDP. Automatic stabilizers reduce contributions when it approaches the cap. Citizen dividends return excess wealth directly to the people who own it — in cash, untracked, constitutionally protected. By 2100, the SWF lives entirely off returns, with contributions phased to zero.

- **The state itself** is bound by the same rules. The Central Council cannot print money without reserve backing. The Regional Boards cannot approve loans without public disclosure. The CWD cannot retain citizen data beyond 30 days. And no agency — ever — may build a system to track what citizens spend their cash on.

Cash is beautiful. BWC is honest. The system is legible. The informal economy is free. The SWF is capped. And the elite have nowhere to hide.

---

**End of Draft 8.0**
