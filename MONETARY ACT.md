...
- **BWC Fees:** as above. Fee bands may be adjusted biennially by both chambers jointly (Territorial + relevant Productive), ratified by the universal citizen vote where the adjustment touches informal-sector activity (Chamber Act §4.3 small-actor protection).
- **Cash Swap:** Cash→BWC free; BWC→Cash 1%.
- **Wartime Fee:** 10% on BWC transactions above $10,000 (5-year cap; renewal requires the Sovereign Council to propose and the Territorial Chamber to ratify at 15/20); cash and cash-to-BWC swaps exempt entirely. The Wartime-Fee pattern — high-quorum, hard sunset, forced re-authorization — is the model for the Tier-2 emergency mode (§1.8).
...
**Consumers of the reference rate (wiring).** Loan and deposit bands (Credit Union Act §4); the special-share dividend premium, reference + 2% after the 7/6/5 step-down (Credit Union Act §3.2); RLP loans at reference + 1% and SWF backstop at reference + 2% (Credit Union Act §6.4); DIP short-term instruments (Credit Union Act §6.5).
...
- **Contributions are carved out of general revenue (not additional),** to avoid double-counting. The national revenue flows fund the sovereign's (capped) operations first (Sovereign Functions Act §7); the surplus seeds the SWF:
  - Financial transactions tax (0.3% on BWC transactions above the $10,000 band): ~$150B/year
  ...
  - **Total annual contributions: ~$363B** (before the remainder's operating draw and before the stabilizer adjustments of §2.3).
...
- **Democratic control:** Freedom Share issuance requires a Regional Board vote (6/11), ensuring the conversion is publicly authorized, not granted in the dark.
...
- **Process & appeal:** CLS makes the determination case by case; a co-op denied may appeal to its Regional Board (6/11), and a grant made over objection may be reviewed by the same.
...
- **SWF seeding:** ~$363B/year from the contribution structure (remainder operations drawn first, surplus to SWF — §2.1 and Sovereign Functions Act §7). No co-op recharge; co-ops contribute to the commons through the FCL profit-sharing waterfall (Section 3).
...
The commons share splits Healthcare : Education : Charity in a 2:1:2 ratio. **Total commons funding ≈ $125B** (Healthcare ~$50B : Education ~$25B : Charity ~$50B per the 2:1:2 split), concentrated in large co-ops while protecting small ones. The healthcare allocation flows through the National Health Board, bound to satisfy the healthcare guarantee (Chamber Act §12.1) before any other distribution.

- **Capital Investment Fund (CCIF):** funds co-op startups and SWF projects; ~7.5% returns; allocated by the Alliance Network (FCL-led, with Territorial Chamber confirmation via Regional Boards).
...
- **Professional Passport Key (PPK):** a citizen-held, air-gapped hardware key holding identity, credentials, professional licensing, voting eligibility, education, **place of residence** (for field-of-membership eligibility, Credit Union Act §1.5), and **firearms-training eligibility** (Chamber Act §11, Floor 7) — and no economic data. Verification is by zero-knowledge proof (prove eligibility without revealing identity). Residence is self-attested and surfaced only as a qualified/not-qualified check, never as tracked location. Firearms training is a qualified/not-qualified attribute; no arms registry may be constructed.
...
| SWF annual contributions | ~$363B (before remainder draw and stabilizer adjustments) | phasing out |
...
| Exchange | Crossroads Exchange: 3 domestic boards + CGCI channel + Global Sound-Capital Segment (Exchange Act 1.2) | same |
...
| Sovereign operating cap | 4% GDP → ~2% as Defense Fund matures | declining |
...
- **The sovereign is thin, funded like a tenant, and constitutionally barred from growth.** It holds four functions on a closed list and is prohibited from everything else. It lives on tariffs and the subsoil, under a cap, with surplus flowing back to citizens. It may never print, never borrow, never tax a home, never surveil a citizen, and never suspend a right — and no war, emergency, or treaty pierces that list.
